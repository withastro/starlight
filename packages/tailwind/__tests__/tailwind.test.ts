...
describe('@layer base', async () => {
	...
	test('configures `--sl-color-*` variables', () => {
		expect(baseLayer).includes('--sl-color-gray-1: var(--color-gray-200);');
		expect(baseLayer).includes('--sl-color-accent: var(--color-accent-600, #4f46e5);');
	});
});
