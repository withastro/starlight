...
/**
 * The frontmatter schema for virtual pages that is also extended to define the default schema for
 * Starlight’s `docs` content collection.
 * The frontmatter schema for virtual pages cannot include the following properties:
 *
 * 	- `editUrl`: Virtual pages cannot be edited.
 * 	- `sidebar`: The sidebar frontmatter prop only works for pages in an autogenerated links
 * group. Virtual page links cannot be autogenerated.
 * 	- `hero`: Virtual pages do not have access to the schema context containing the Astro image
 * helper.
 */
export const StarlightVirtualFrontmatterSchema = z.object({
...
export type StarlightVirtualFrontmatter = z.input<typeof StarlightVirtualFrontmatterSchema>;

/** Default content collection schema for Starlight’s `docs` collection. */
const StarlightFrontmatterSchema = (context: SchemaContext) =>
	StarlightVirtualFrontmatterSchema.extend({
		/**
