...
function getLocaleDir(locale: Intl.Locale): 'ltr' | 'rtl' {
	...
	// of right-to-left languages. The two `if` branches above narrow `locale` to
	// `never` under TypeScript v6's newer DOM lib (which knows about both
	// `textInfo` and `getTextInfo`), so ts-ignore the property access to stay
	// compatible with both TS 5 (where `.language` resolves) and TS 6 (where
	// the narrowed type no longer carries it).
	// @ts-expect-error - `locale.language` is reachable on all runtimes; the narrowed
	// type under TS 6 drops it because of the `in` guards above.
