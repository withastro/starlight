...
export function getRoutes(docs: StarlightDocsEntry[]): Route[] {
  docs = docs.map(({ slug, ...entry }) => ({ ...entry, slug: normalizeIndexSlug(slug) }));
  ...
  if (config.isMultilingual) {
    ...
    for (const key in config.locales) {
      ...
      const localeDocs = getLocaleDocs(docs, locale);
...
export function getPaths(routes: Route[]): Path[] {
