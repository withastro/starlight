...
(Se la tua lingua predefinita è il tuo [locale radice](#locale-radice), puoi saltare questo.)
...
Consulta la [guida "Blocchi di codice"](/guides/authoring-content/#code-blocks) per imparare come utilizzare la sintassi di Expressive Code nel tuo contenuto Markdown e MDX.
...
Quando `true`, le variabili CSS di Starlight vengono utilizzate per i colori degli elementi dell'interfaccia utente dei blocchi di codice (sfondi, pulsanti, ombre, ecc.), corrispondenti al [tema di colore del sito](/guides/css-and-tailwind/#theming).
...
Le voci in `head` vengono convertite direttamente in elementi HTML e non passano attraverso l'elaborazione [script](https://docs.astro.build/en/guides/client-side-scripts/#script-processing) o [style](https://docs.astro.build/en/guides/styling/#styling-in-astro) di Astro.
Se hai bisogno di importare risorse locali come script, stili o immagini, [sovrascrivi il componente Head](/guides/overriding-components/#reuse-a-built-in-component).
...
Una pagina può sovrascrivere questa impostazione o il testo del link e/o l'URL utilizzando i campi frontmatter [`prev`](/reference/frontmatter/#prev) e [`next`](/reference/frontmatter/#next).
