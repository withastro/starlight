...
2. Aggiungi il percorso del tuo logo come opzione [`logo.src`](/reference/configuration/#logo) di Starlight in `astro.config.mjs`:
...
Per impostazione predefinita, le pagine Starlight utilizzano un layout con una barra laterale di navigazione globale e un sommario che mostra le intestazioni della pagina corrente.

Puoi applicare un layout di pagina più ampio senza barre laterali impostando [`template: splash`](/reference/frontmatter/#template) nel frontmatter di una pagina.
...
tititletolo: La mia pagina di destinazione
template: spruzzo
---
```
...
Starlight visualizza un sommario su ogni pagina per consentire ai lettori di passare più facilmente all'intestazione che stanno cercando.
Puoi personalizzare, o addirittura disabilitare, il sommario a livello globale nell'integrazione Starlight o pagina per pagina in Frontmatter.

Per impostazione predefinita, le intestazioni `<h2>` e `<h3>` sono incluse nel sommario. Modifica i livelli delle intestazioni da includere in tutto il sito utilizzando le opzioni `minHeadingLevel` e `maxHeadingLevel` nel tuo [global `tableOfContents`](/reference/configuration/#tableofcontents). Sostituisci queste impostazioni predefinite su una singola pagina aggiungendo le proprietà [frontmatter `tableOfContents`](/reference/frontmatter/#tableofcontents) corrispondenti:
...
title: pagina con solo H2 nel sommario
summary:
  minHeadingLevel: 2
  maxHeadingLevel: 2
---
...
Disabilita completamente il sommario impostando l'opzione `tableOfContents` su `false`:
...
defineConfig({
	integrations: [
		starlight({
			title: 'Documenti con sommario disabilitato a livello globale',
...
Starlight dispone del supporto integrato per aggiungere collegamenti ai tuoi account di social media all'intestazione del sito tramite l'opzione [`social`](/reference/configuration/#social) nell'integrazione Starlight.
...
Per abilitare i collegamenti di modifica, imposta [`editLink.baseUrl`](/reference/configuration/#editlink) sull'URL utilizzato per modificare il tuo repository nella configurazione dell'integrazione Starlight.
...
Puoi utilizzare tutto il layout di pagina e le tecniche di personalizzazione di Starlight nella tua pagina 404. Ad esempio, la pagina 404 predefinita utilizza il componente [`splash` template](#page-layout) e [`hero`](/reference/frontmatter/#hero) in frontmatter:
...
Per applicare il font che hai impostato al tuo sito, utilizza il nome del font scelto in un [file CSS personalizzato](/guides/css-and-tailwind/#custom-css-styles).
...
Segui le [istruzioni CSS personalizzate](/guides/css-and-tailwind/#custom-css-styles) per aggiungere i tuoi stili al tuo sito.
