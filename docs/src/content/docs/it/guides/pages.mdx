...
- La proprietà del frontmatter [`sidebar`](/reference/frontmatter/#sidebar) per personalizzare come appare la pagina in [gruppi di link autogenerati](/reference/configuration/#sidebar) non è disponibile. Le pagine che utilizzano il componente `<StarlightPage />` non fanno parte di una collezione e non possono essere aggiunte a un gruppo di barra laterale autogenerato.
