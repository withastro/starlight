...
- La propiedad [`sidebar`](/es/reference/frontmatter/#sidebar) no es compatible. En el frontmatter de Markdown, esta opción permite la personalización de [grupos de enlaces autogenerados](/es/reference/configuration/#sidebar), lo cual no es aplicable a las páginas que utilizan el componente `<StarlightPage />`.
