...
Los asides (también conocidos como "admoniciones" o "llamadas") son útiles para mostrar información secundaria junto al contenido principal de una página.

Un `<Aside>` puede tener un `type` opcional de `note` (el valor predeterminado), `tip`, `caution` o `danger`. Establecer un atributo `title` anula el título predeterminado del aside.
...
<Aside>Un aside predeterminado sin un título personalizado.</Aside>

<Aside type="caution" title="Watch out!">
	Un aside de advertencia *con* un título personalizado.
</Aside>

<Aside type="tip">

Otro contenido también es compatible en los asides.
...
Starlight también proporciona una sintaxis personalizada para renderizar asides en Markdown y MDX como una alternativa al componente `<Aside>`.
Consulta la guía [“Creación de contenido en Markdown”](/es/guides/authoring-content/#asides) para obtener más detalles de la sintaxis personalizada.
