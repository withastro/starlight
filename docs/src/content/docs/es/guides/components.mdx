...
Un `<LinkCard>` requiere un `title` y un atributo [`href`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a#href). Opcionalmente puedes incluir una breve `description` u otros atributos de enlace como `target`.
...
<CardGrid>
	...
	/>
	<LinkCard title="Componentes" href="/guides/components/" />
</CardGrid>
```

El código de arriba genera lo siguiente en la página:
