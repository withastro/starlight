...
Por defecto, Starlight generará automáticamente una barra lateral basada en la estructura del sistema de archivos de su documentación, utilizando la propiedad `title` de cada archivo como entrada de la barra lateral.
...
Para configurar los enlaces de tu barra lateral [enlaces](#enlaces) y [grupos de enlaces](#grupos) (dentro de un encabezado plegable), usa la propiedad [`starlight.sidebar`](/es/reference/configuration/#sidebar) en `astro.config.mjs`.
...
starlight({
	sidebar: [
		// Un enlace a la guía CSS & Styling.
...
Las opciones del frontmatter de la barra lateral te permiten establecer un [etiqueta personalizada](/es/reference/frontmatter/#label) o agregar una [insignia](/es/reference/frontmatter/#badge), [esconder](/es/reference/frontmatter/#hidden) un enlace de la barra lateral o definir un [peso de ordenación personalizado](/es/reference/frontmatter/#order).
