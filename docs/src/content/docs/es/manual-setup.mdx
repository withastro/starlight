...
The quickest way to create a new Starlight site is using `create astro` as shown in the [Getting Started guide](/getting-started/#creating-a-new-project).
La manera más rápida de crear un nuevo sitio Starlight es usando `create astro` como se muestra en la [Guía de Inicio Rápido](/getting-started/#creating-a-new-project).
Si quieres agregar Starlight a un proyecto Astro existente, esta guía te explicará como hacerlo.

## Configuración de Starlight

Para seguir esta guía, necesitarás un proyecto Astro existente.
...
Esto instalará las dependencias necesarias y agregará Starlight al arreglo `integrations` en el archivo de configuración de Astro.
...
Si tienes un proyecto Astro existente, puedes usar Starlight para agregar rápidamente una sección de documentación a tu sitio.
...
Por el momento, Starlight no soporta [despliegue SSR](https://docs.astro.build/es/guides/server-side-rendering/) usando los adaptadores de servidor de Astro. Esperamos poder soportar esto pronto.
