...
Wenn du das Aussehen deiner Starlight-Site anpassen willst, findest du in dieser Anleitung alle nötigen Informationen.

## Dein Logo hinzufügen

Das Hinzufügen eines eigenen Logos im Header ist ein schneller Weg, um einer Starlight-Site dein individuelles Branding zu geben.
...
export default defineConfig({
	...
	integrations: [starlight({ title: 'Webseite mit Sitemap' })],
});
```
...
Du kansnt das Inhaltsverzeichnis global in der Starlight-Integration oder seitenweise im Frontmatter anpassen - oder sogar deaktivieren.

Standardmäßig werden die Überschriften `<h2>` und `<h3>` in das Inhaltsverzeichnis aufgenommen. Änder die Überschriftsebenen für die gesamte Website mit den Optionen `minHeadingLevel` und `maxHeadingLevel` in deinem [globalen `tableOfContents`](/de/reference/configuration/#tableofcontents). Überschreibe diese Standardwerte auf einer individuellen Seite, indem du die entsprechenden [Frontmatter `tableOfContents`](/de/reference/frontmatter/#tableofcontents) Eigenschaften hinzufügst:
...
Derzeit werden Links zu Bitbucket, Codeberg, CodePen, Discord, GitHub, GitLab, Gitter, Instagram, LinkedIn, Mastodon, Microsoft Teams, Stack Overflow, Threads, Twitch, Twitter und Youtube unterstützt.
...
Du kannst dies anpassen, indem du eine `404.md` (oder `404.mdx`) Datei zu deinem `src/content/docs/` Verzeichnis hinzufügen:
...
Wenn du bereits über Schriftdateien verfügen, folge der [Anleitung "Lokale Schriftartendateien einrichten"](#lokale-schriftartendateien-einrichten).
...
2.  Installiere das Paket für deine gewählte Schriftart.
    Du findest den Namen des Pakets, indem du auf der Fontsource-Webseite auf `Installieren` klicken.
...
3.  Füge die CSS-Dateien von Fontsource zum Array `customCss` von Starlight in `astro.config.mjs` hinzu:
    ...
    Fontsource liefert mehrere CSS-Dateien für jede Schriftart. Siehe die [Fontsource-Dokumentation](https://fontsource.org/docs/getting-started/install#4-weights-and-styles) über das Einbinden verschiedener Schnitte und Stile, um zu verstehen, welche zu verwenden sind.

### Schriftarten verwenden

Um deine eingerichtete Schriftart auf deiner Website anzuwenden, verwende den Namen der gewählten Schriftart in einer [custom CSS-Datei](/guides/css-and-tailwind/#custom-css-styles).
...
Folge den [custom CSS Anleitungen](/de/guides/css-and-tailwind/#benutzerdefinierte-css-stile), um deine Stile zu deiner Website hinzuzufügen.
