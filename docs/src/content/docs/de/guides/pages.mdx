...
Aufgrund der Art und Weise, wie die [Importreihenfolge](https://docs.astro.build/de/guides/styling/#import-reihenfolge) in Astro funktioniert, muss die Komponente `<StarlightPage />` als erstes in deiner Datei importiert werden, um die [Kaskadenschichten](/guides/css-and-tailwind/#cascade-layers) einzurichten, die Starlight intern verwendet, um die Reihenfolge seiner Stile zu verwalten.
