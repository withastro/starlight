...
Du kannst deine Starlight-Webseite mit benutzerdefinierten CSS-Dateien gestalten oder das Starlight Tailwind-Plugin verwenden.

## Benutzerdefinierte CSS-Stile

Passe die Stile deiner Starlight-Seite an, indem du zusätzliche CSS-Dateien bereitstellst, um die Standard-Stile von Starlight zu verändern oder zu erweitern.
...
2. Füge den Pfad zu deiner CSS-Datei in Starlight's `customCss`- Array in `astro.config.mjs` ein:
...
Starlight bietet ein komplementäres Tailwind-Plugin, um Tailwind für die Kompatibilität mit den Stilen von Starlight zu konfigurieren.
...
- Verwendet Tailwind [Themenfarben und Schriftarten](#styling-starlight-with-tailwind) in Starlights UI.
- Deaktiviert Tailwinds [Preflight](https://tailwindcss.com/docs/preflight) Zurücksetzungs-Stile, während selektiv wesentliche Teile von Preflight wiederhergestellt werden, die für die Grenznutzungsklassen von Tailwind benötigt werden.
...
Wenn du bereits eine Starlight-Site hast und Tailwind-CSS hinzufügen möchtest, folge dieser Anleitung.
...
3.  Erstelle eine CSS-Datei für die Basisstile von Tailwind, zum Beispiel unter `src/tailwind.css`:
...
4.  Aktualisiere deine Astro-Konfigurationsdatei, um deine Tailwind-Basisstile zu verwenden und deaktiviere die Standard-Basisstile:
    ...
    export default defineConfig({
    	integrations: [
    		starlight({
    			...
    			customCss: [
    				// Pfad zu deinen Tailwind-Basisstilen
    				'./src/tailwind.css',
    			],
    		}),
    		tailwind({
    			// Deaktiviere die Basisstile
...
5.  Füge das Starlight Tailwind Plugin zu `tailwind.config.cjs` hinzu:
...
Starlight verwendet Werte aus deiner [Tailwind theme config](https://tailwindcss.com/docs/theme) in der Benutzeroberfläche.

Falls gesetzt, überschreiben die folgenden Optionen die Standard-Stile von Starlight:
...
>
	<Fragment slot="css-docs">
		Füge das folgende CSS in deinem Projekt in einer [custom CSS
		Datei](#custom-css-styles), um dieses Thema auf deiner Webseite anzuwenden.
	</Fragment>
	<Fragment slot="tailwind-docs">
		Das folgende Beispiel
		[Tailwind-Konfigurationsdatei](#styling-starlight-with-tailwind) enthält
