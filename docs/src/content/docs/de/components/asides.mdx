...
Ein `<Aside>` kann ein optionales [`Typ`](#type) Attribute haben, welches die Farbe, das Symbol und den Standardtitel der Nebenbemerkung steuert.
...
Überschreibe die voreingestellten Seitentitel mit dem Attribut [`title`](#title).
...
Der Titel der anzuzeigenden Nebenbemerkung.
