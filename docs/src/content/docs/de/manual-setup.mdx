...
Erstelle oder aktualisiere die Inhaltskonfigurationsdatei und füge eine `docs`-Sammlung hinzu, die Starlight's `docsLoader` und `docsSchema` verwendet:
...
Starlight unterstützt auch die Option [`legacy.collections`] (https://docs.astro.build/en/reference/legacy-flags/), bei dem Sammlungen mit der Legacy-Implementierung von Inhaltssammlungen behandelt werden.
