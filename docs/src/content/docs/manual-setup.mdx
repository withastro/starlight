...
If your Astro project is configured to use SSR, you probably don't need to change anything in Starlight and it will use the same settings.

Astro allows pages to opt in or out of prerendering when SSR is enabled. Starlight pages will follow the default for your Astro configuration, prerendering when output mode is set to `hybrid` and using SSR when set to `server`. You can override this behavior by setting the [`prerender` config option](/reference/configuration/#prerender).
