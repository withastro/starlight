...
Um `<Aside>` pode ter um `type` opcional de `note` (o padrão, nota), `tip` (dica), `caution` (cuidado) ou `danger` (perigo). A definição de um atributo `title` substitui o título padrão do aside.
