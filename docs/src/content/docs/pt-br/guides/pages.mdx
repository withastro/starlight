...
```md
Todas as páginas do Starlight compartilham um [conjunto comum de propriedades de frontmatter](/reference/pt-br/frontmatter/) personalizáveis para controlar como a página aparece:
...
O [`title`](/reference/frontmatter/#title-obrigatorio) é requerido e todas as outras propriedades são opcionais.
...
**tipo:** `boolean`  
**padrão:** `false` se [`frontmatter.template`](/reference/frontmatter/#template) é `'splash'`, caso contrário é `true`
