...
Une `<Card>` nécessite un `title` et peut optionellement inclure un attribut `icon` fixé au nom de [l'une des icônes intégrées de Starlight](#all-icons).
...
Chaque `<Icon>` nécessite un [`name`](#all-icons) et peut optionellement inclure un attribut `label`, `size` et `color`.
