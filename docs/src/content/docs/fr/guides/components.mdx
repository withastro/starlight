...
Les encarts (également connues sous le nom de "aside" ou encore "élément en marge") sont utiles pour afficher des informations secondaires à côté du contenu principal d'une page.

Un `<Aside>` peut avoir un `type` optionnel de `note` (par défaut), `tip`, `caution` ou `danger`. La définition d'un attribut `title` remplace le titre par défaut de l'encart.
...
<Aside type="caution" title="Watch out!">
  Un avertissement est mis en encart *avec* un titre personnalisé.
</Aside>
...
Starlight fournit également une syntaxe personnalisée pour rendre les encarts dans Markdown et MDX comme alternative au composant `<Aside>`.
Voir le guide de [création de contenu en Markdown"](/fr/guides/authoring-content/#asides) pour plus de détails sur la syntaxe personnalisée.
