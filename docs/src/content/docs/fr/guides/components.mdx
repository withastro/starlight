...
Par défaut, le badge utilise la couleur d'accentuation du thème de votre site. Pour utiliser une couleur de badge intégrée, définissez l'attribut `variant` à l'une des valeurs suivantes : `note` (bleu), `tip` (violet), `danger` (rouge), `caution` (orange), ou `success` (vert).

L'attribut `size` (par défaut : `small`) contrôle la taille du texte du badge. Les attributs `medium` et `large` sont également disponibles pour afficher un badge plus grand.

Pour une personnalisation plus poussée, utilisez d'autres attributs `<span>` tels que `class` ou `style` avec du CSS personnalisé.
