...
Pour les scénarios i18n plus avancés, Starlight prend également en charge la configuration de l'internationalisation à l'aide de l'option de [configuration `i18n` dans Astro](https://docs.astro.build/fr/guides/internationalization/#configure-i18n-routing).
...
## Accès à la langue locale actuelle

Vous pouvez utiliser [`Astro.currentLocale`](https://docs.astro.build/fr/reference/api-reference/#astrocurrentlocale) pour lire les paramètres linguistiques actuels dans les composants `.astro`.

L'exemple suivant lit la langue locale courante et l'utilise pour générer un lien vers une page d'information dans la langue courante :
