...
Par défaut, les pages sont triées par ordre alphabétique selon le fichier [`slug`](/fr/reference/overrides/#slug).
...
sidebar:
  ...
  # Définit un ordre personnalisé pour le lien (les nombres plus petits sont affichés plus haut)
  order: 2
...
starlight({
	sidebar: [
		{
			label: 'Guides',
			// Rétracte le groupe et ses sous-groupes générés automatiquement par défaut.
			collapsed: true,
			autogenerate: { directory: 'guides' },
		},
	],
