...
Pour en savoir plus sur les barres latérales générées automatiquement, consultez la section sur les [liens générés automatiquement](#liens-générés-automatiquement).
...
Starlight peut générer automatiquement des liens dans votre barre latérale en fonction d'un répertoire de votre documentation.
Cela est utile lorsque vous ne souhaitez pas entrer manuellement chaque élément de la barre latérale dans un groupe.

Par défaut, les pages sont triées par ordre alphabétique selon l'[`id`](/fr/reference/route-data/#id) du fichier.

Ajoutez des liens générés automatiquement en utilisant un objet avec la propriété `autogenerate`.
La configuration de `autogenerate` doit spécifier le répertoire (`directory`) à utiliser pour les entrées de la barre latérale.
Par exemple, avec la configuration suivante :
...
starlight({
	sidebar: [
		{
			label: 'Constellations',
			// Générer automatiquement des liens pour le répertoire 'constellations'.
...
Un groupe avec des liens générés automatiquement incluant une page avec le frontmatter ci-dessus générera la barre latérale suivante :
...
:::note
La configuration de `sidebar` dans le frontmatter est uniquement utilisée pour les liens générés automatiquement et les liens de documentation définis avec la propriété `slug`. Elle ne s'applique pas aux liens définis avec la propriété `link`.
:::

## Badges

Les liens et les groupes peuvent également inclure une propriété `badge` pour afficher un badge à côté de leur étiquette.
...
starlight({
	sidebar: [
		...
		// Un groupe avec un badge « Obsolète ».
		{
			label: 'Lunes',
			badge: 'Obsolète',
			items: [{ autogenerate: { directory: 'lunes' } }],
		},
	],
});
```
...
Personnalisez les attributs HTML de tous les [liens générés automatiquement](#liens-générés-automatiquement) en définissant la propriété `attrs` dans la configuration `autogenerate`.
...
starlight({
	sidebar: [
		{
			autogenerate: {
				// Génère automatiquement des liens pour le répertoire 'constellations'.
				directory: 'constellations',
				// Met en italique toutes les étiquettes de liens générés automatiquement.
...
<SidebarPreview
	config={[
		...
		{ label: 'Centaure', link: '', attrs: { style: 'font-style: italic' } },
		{
			label: 'saisonnieres',
			items: [
				{
...
Les [sous-groupes générés automatiquement](#liens-générés-automatiquement) peuvent également être rétractés par défaut en définissant la propriété `autogenerate.collapsed` avec `true`.
			...
			items: [
				{
					autogenerate: {
						directory: 'constellations',
						// Rétracte les sous-groupes générés automatiquement par défaut.
						collapsed: true,
					},
				},
			],
