...
Vous pouvez ajouter la prise en charge de Markdoc en installant l'intégration expérimentale [Astro Markdoc integration](https://docs.astro.build/fr/guides/integrations-guide/markdoc/).
...
### Sécurité de la typologie du frontmatter

Toutes les pages Starlight partagent un [ensemble commun de propriétés de la page d'accueil](/fr/reference/frontmatter/) personnalisable qui permet de contrôler l'apparence de la page :

```md
---
title: Bonjour tout le monde !
description: Voici une page de mon site alimenté par Starlight
---
```
...
Le répertoire `src/pages/` utilise le [routage basé sur les fichiers d'Astro](https://docs.astro.build/fr/basics/astro-pages/#file-based-routing) et inclut le support des fichiers `.astro` parmi d'autres formats de pages.
...
La propriété [`title`](/fr/reference/frontmatter/#title-required) est obligatoire et toutes les autres propriétés sont optionnelles.
...
Indiquer si cette page utilise [contenu en fallback](/fr/guides/i18n/#fallback-content) parce qu'il n'y a pas de traduction pour la langue actuelle.
