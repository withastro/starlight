...
Actuellement, les liens vers Bitbucket, Codeberg, Codepen, Discord, GitHub, GitLab, Gitter, LinkedIn, Mastodon, Microsoft Teams, Threads, Twitch, Twitter et Youtube sont pris en charge.
   ...
   export default defineConfig({
   	integrations: [
   		starlight({
   			...
   			customCss: [
   				// Chemin relatif vers votre fichier CSS personnalisé, Commence par /
   				'/src/styles/custom.css',
   			],
   		}),
   	],
...
Vous pouvez voir toutes les propriétés CSS personnalisées utilisées par Starlight que vous pouvez définir pour personnaliser votre site dans le [fichier `props.css` sur GitHub](https://github.com/withastro/starlight/blob/main/packages/starlight/style/props.css).
...
Suivez les [instructions de CSS personnalisé](#styles-css-personnalises) pour ajouter vos styles à votre site.
