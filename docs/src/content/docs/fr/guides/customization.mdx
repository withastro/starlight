...
Actuellement, les liens vers Discord, GitHub, Mastodon et Twitter sont pris en charge.
...
Starlight peut afficher un lien "Modifier la page" dans le pied de page de chaque page.
...
Pour activer les liens de modification, définissez l'option de configuration [`editLink.baseUrl`](/reference/configuration/#editlink) en lui assignat l'URL utilisée pour modifier votre référentiel. Cela se passe dans la configuration de l'intégration Starlight.
...
Vous pouvez utiliser toutes les techniques de mise en page et de personnalisation de Starlight dans votre page 404. Par exemple, la page 404 par défaut utilise les composants [`splash` template](#page-layout) et [`hero`](/reference/frontmatter/#hero) dans son frontmatter :
...
Personnalisez les styles appliqués à votre site Starlight en créant des fichiers CSS complémentaires pour modifier ou étendre les styles par défaut de Starlight.

1. Ajoutez un fichier CSS dans votre répertoire `src/`.
   Par exemple, vous pouvez remplacer la teinte d'accentuation bleue par défaut de Starlight par du violet :
...
2. Ajoutez le chemin vers votre fichier CSS au tableau `customCss` de Starlight dans `astro.config.mjs` :
   ...
   export default defineConfig({
     integrations: [
       starlight({
         title: "Site avec CSS personnalisé",
         customCss: [
           // Chemin vers votre fichier CSS personnalisé, Commence par /
           "/src/styles/custom.css",
         ],
       }),
     ],
...
Pour utiliser Google Fonts, suivez le [Guide de configuration de Fontsource](#set-up-a-fontsource-font).
   ...
   export default defineConfig({
     integrations: [
       starlight({
         ...
         customCss: [
           // Chemin vers votre fichier CSS @font-face.
           "/src/fonts/font-face.css",
         ],
       }),
     ],
