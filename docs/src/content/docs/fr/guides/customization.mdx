...
Si votre projet nécessite une mise en page 404 entièrement personnalisée, vous pouvez créer une route `src/pages/404.astro` et définir l'option de configuration [`disable404Route`](/fr/reference/configuration/#disable404route) pour désactiver la route par défaut de Starlight :
...
export default defineConfig({
	integrations: [
		starlight({
			title: 'Docs avec 404 personnalisé',
