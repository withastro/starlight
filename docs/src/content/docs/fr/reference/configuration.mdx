...
Affectez un objet à `pagefind`pour configurer le client de recherche [Pagefind](https://pagefind.app/) :
- Consultez [“Customize Pagefind's result ranking”](https://pagefind.app/docs/ranking/) dans la documentation de Pagefind pour plus de détails à propos des options `pagefind.ranking` pour contrôler comment le classement des résultats de recherche est calculé,
- Consultez [“Searching multiple sites”](https://pagefind.app/docs/multisite/) dans la documentation de Pagefind pour plus de détails à propos des options `pagefind.mergeIndex` pour configurer plusieurs site comme source de données de la recherche.
