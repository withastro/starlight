...
Définit les répertoires supplémentaires dans lesquels les fichiers doivent être traités par le pipeline Markdown de Starlight.
...
Le traitement de Starlight inclut la prise en charge les [liens d'ancrage cliquables des en-têtes](#headinglinks), la [syntaxe de directive Markdown pour les encarts](/fr/guides/authoring-content/#encarts) et la prise en charge RTL des blocs de code.
