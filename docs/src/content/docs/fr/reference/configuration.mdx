...
Une barre latérale est un tableau de liens, de groupes de liens et d'entrées générées automatiquement.
Les éléments de la barre latérale sont définis en utilisant l'une des propriétés suivantes :

- `link` — un lien vers une URL spécifique, par exemple `'/home'` ou `'https://example.com'`. Un lien doit également avoir une étiquette (`label`).

- `slug` — une référence à une page interne, par exemple `'guides/getting-started'`. Le titre de la page liée sera utilisé comme étiquette par défaut.

- `items` — un tableau contenant d'autres liens, sous-groupes et entrées générées automatiquement pour la barre latérale. Un groupe doit également avoir une étiquette (`label`).

- `autogenerate` — un objet spécifiant un répertoire de votre documentation à partir duquel générer automatiquement des liens et des sous-groupes. Les entrées générées automatiquement peuvent être placées dans le tableau `items` d'un groupe.
...
Les sous-groupes générés automatiquement sont également développés par défaut. Définissez la propriété `autogenerate.collapsed` pour les rétracter.
...
type SidebarItem =
	...
	| {
			// Liens et sous-groupes générés automatiquement
			autogenerate: {
				directory: string;
				collapsed?: boolean;
				attrs?: Record<string, string | number | boolean | undefined>;
