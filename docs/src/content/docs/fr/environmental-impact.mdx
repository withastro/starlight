...
Ces tests réalisés avec [Website Carbon Calculator][wcc] comparent des pages similaires construites avec différents outils.

import CarbonComparison from '../../../components/carbon-comparison.astro';

<CarbonComparison
	labels={{
		framework: 'Framework',
		co2: 'CO₂ par page visitée',
		rating: 'Notation',
		note: "Données collectées le {{date}}. Cliquez sur l'un des liens pour voir les chiffres actualisés.",
	}}
/>
