...
<Aside slot="preview">
Incluez des informations supplémentaires non essentielles dans un `<Aside>`.
</Aside>
...
Affichez un encart à l'aide du composant `<Aside>`.

Un `<Aside>` peut avoir un attribut facultatif [`type`](#type), qui contrôle la couleur, l'icône et le titre par défaut de l'encart.
...
<Aside>Un peu de contenu en encart.</Aside>
...
Starlight fournit également une syntaxe personnalisée pour afficher les encarts dans Markdown et MDX comme alternative au composant `<Aside>`.
Voir le guide [« Authoring Content in Markdown »](/fr/guides/authoring-content/#encarts) pour plus de détails sur la syntaxe personnalisée.

### Utilisation de titres personnalisés
...
<Aside type="caution" title="Attention !">
	Un avertissement est mis de côté *avec* un titre personnalisé.
</Aside>
```
...
- L'encarts `danger` est rouge et affiche une icône d'avertissement octogonale.
