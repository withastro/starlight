...
2. Додайте шлях до свого логотипу як параметр Starlight [`logo.src`](/reference/configuration/#logo) у `astro.config.mjs`:
...
Ви можете застосувати ширший макет сторінки без бічних панелей, встановивши [`template: splash`](/reference/frontmatter/#template) у передній частині сторінки.
Це особливо добре працює для цільових сторінок, і ви можете побачити його в дії на [домашній сторінці цього сайту](/).
...
За замовчуванням заголовки `<h2>` і `<h3>` включені до змісту. Змініть, які рівні заголовків включити на весь сайт, використовуючи параметри `minHeadingLevel` і `maxHeadingLevel` у вашому [глобальному `tableOfContents`](/reference/configuration/#tableofcontents). Перевизначте ці значення за замовчуванням на окремій сторінці, додавши відповідні властивості [frontmatter `tableOfContents`](/reference/frontmatter/#tableofcontents):
...
Starlight має вбудовану підтримку для додавання посилань на ваші облікові записи соціальних мереж до заголовка сайту за допомогою параметра [`social`](/reference/configuration/#social) в інтеграції Starlight.
...
Щоб увімкнути посилання для редагування, установіть [`editLink.baseUrl`](/reference/configuration/#editlink) на URL-адресу, яка використовується для редагування вашого сховища в конфігурації інтеграції Starlight.
...
Ви можете використовувати всі методи компонування та налаштування Starlight на своїй сторінці 404. Наприклад, стандартна сторінка 404 використовує компонент [`splash` template](#page-layout) і [`hero`](/reference/frontmatter/#hero) у frontmatter:
...
Щоб застосувати шрифт, який ви налаштували, до свого сайту, використовуйте назву вибраного шрифту в [спеціальному файлі CSS](/guides/css-and-tailwind/#custom-css-styles).
