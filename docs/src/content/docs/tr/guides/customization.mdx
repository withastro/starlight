---
title: Starlight'ı İsteğe Uyarlama
...
2. `astro.config.mjs` içerisine Starlight'ın [`logo.src`](/reference/configuration/#logo) ayarı olarak logonuzun dosya yolunu ekleyin:
...
Sayfanın önbölümü içerisinde [`template: splash`](/reference/frontmatter/#template) ayarlayarak kenar çubuksuz daha geniş sayfa şablonu oluşturabilirsiniz.
...
Varsayılan olarak, `<h2>` ve `<h3>` etiketleri içindekiler listesine dahil edilir. [Genel `tableOfContents`](/reference/configuration/#tableofcontents) içerisinde `minHeadingLevel` ve `maxHeadingLevel` seçeneklerini kullanarak site genelindeki başlık seviyelerini değiştiri. Bu varsayılan değerleri tekil bir sayfanın [önbölümüne `tableOfContents`](/reference/frontmatter/#tableofcontents) karşılık gelen niteliklerini ekleyerek değiştirin:
...
defineConfig({
	integrations: [
		starlight({
			title: 'İçindekiler tablosunun genel olarak etkisizleştirildiği dokümantasyon',
			tableOfContents: false,
		}),
	],
});
...
Starlight'ın, site başlığına Starlight entegrasyonu içindeki [`social`](/reference/configuration/#social) seçeneğiyle sosyal medya hesaplarınızın bağlantısını eklemek için yerleşik desteği vardır.

[Yapılandırma Referansında](/reference/configuration/#social) desteklenen tüm bağlantı ikonlarının bulunduğu listeyi bulabilirsiniz.
...
Sayfa düzenleme bağlantıları etkinleştirmek için, Starlight entegrasyon yapılandırmasında [`editLink.baseUrl`](/reference/configuration/#editlink) seçeneğine düzenlenecek reponuzu ekleyin.
...
404 sayfanızda Starlight'ın tüm sayfa şablonlarını ve isteğe uyarlama tekniklerini kullanabilirsiniz. Örneğin, varsayılan 404 sayfası [`splash` şablonunu](#sayfa-şablonu) ve önblümdeki [`hero`](/reference/frontmatter/#hero) bileşenini kullanır:
...
Projeniz tamamen isteğe uyarlanmış 404 şablonuna ihtiyaç duyarsa, `src/pages/404.astro` dizini oluşturabilir ve [`disable404Route`](/reference/configuration/#disable404route) yapılandırma seçeneğini kullanarak Starlight'ın varsayılan yöneltmesini etkisizleştirebilirsiniz:
...
Halihazırda yazı karakteri dosyalarınız varsa, [lokal ayarlama rehberini](#set-up-local-font-files) takip edin.
Google Fonts kullanmak için [Fontsource ayarlama rehberini](#set-up-a-fontsource-font) takip edin.
...
Sitenize ayarladığınız yazı karakterini uygulamak için seçtiğiniz yazı karakterinin ismini [özel CSS dosyasında](/guides/css-and-tailwind/#custom-css-styles) kullanın.
...
[isteğe uyarlanmış CSS talimatlarını](/guides/css-and-tailwind/#custom-css-styles) sitenize kendi stilinizi eklemek için takip edin.
