...
- Tailwind'in [tema renkler ve yazı karakterlerini](#styling-starlight-with-tailwind) Starlight'ın kullanıcı arayüzünde kullanır.
...
<ThemeDesigner
	labels={{
		...
		preview: {
			...
			lightMode: 'Açık Mod',
			bodyText:
				'Sayfa metni arkaplanla yüksek kontrastlı gri tonda görünür.',
			linkText: 'Bağlantılar renklendirilmiştir.',
			dimText: 'İçindekiler metni gibi bazı metinler daha düşük kontrasta sahiptir.',
			inlineCode: 'Satır için kod belirgin arkaplana sahiptir.',
		},
	}}
>
	<Fragment slot="css-docs">
	Bu temayı sitenize uygulamak için, [isteğe uyarlanmış CSS
		dosyası içine](#custom-css-styles) aşağıdaki CSS'i dahil edin ve projenize ekleyin.
	</Fragment>
	<Fragment slot="tailwind-docs">
	Aşağıdaki [Tailwind yapılandırma dosyasına ait](#styling-starlight-with-tailwind) örnek, yapılandırma nesnesi içerisinde oluşturulmuş `accent` ve `gray` renk paletlerini dahil eder.
	</Fragment>
</ThemeDesigner>
