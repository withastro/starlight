...
You can read the current locale from the `Astro` global that is available in all contexts in `.astro` files with the [`currentLocale`](https://docs.astro.build/en/reference/api-reference/#astrocurrentlocale) property.
