...
Use the `<FileTree>` component to display a file tree in your documentation representing the structure of a directory.

A `<FileTree>` requires an [unordered Markdown list](https://www.markdownguide.org/basic-syntax/#unordered-lists) of files and directories.
Directories can either end with a `/` or contain a nested list of files and directories.

The following syntax can be used to customize the appearance of the file tree:

- If a file or directory name is bolded, e.g. `**README.md**`, it will be highlighted in the file tree.
- Comments can be appended after a file or directory name to provide additional information.
- Placeholder files and directories can be added by using either `...` or `…` as the name.
