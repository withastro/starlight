...
You can also include other `<span>` attributes inside the `attrs` property such as `class` or `style` to further customize the badge.
...
<Badge text="Custom" attrs={{ style: 'font-style: italic;' }}/>
```

The code above generates the following on the page:

<Badge text="New" variant="tip" size="small" class="" />
<Badge text="Deprecated" variant="caution" size="medium" />
<Badge text="Starlight" variant="note" size="large" />
<Badge
	text="Custom"
