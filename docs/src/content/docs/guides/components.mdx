...
A `<LinkCard>` requires a `heading` and an [`href`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a#href) attribute. You can optionally include a short `description` or other link attributes such as `target`.
...
<CardGrid>
	<LinkCard heading="Authoring Markdown" href="/guides/authoring-content/" />
	<LinkCard heading="Components" href="/guides/components/" />
</CardGrid>
```
