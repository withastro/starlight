...
Use the `GeneratedIndex` component to build an list `LinkCard` for the pages in a specified directory. This respects the properties specified in the `sidebar` frontmatter of the pages in that directory such as `label`, `order`, and `hidden`.
