...
Starlight provides three options for dark/light theme choise: `dark`, `light` and `auto`. `auto` uses users default environment settings to determine dark or ligh theme.

Starlight provides settings to change the default loaded theme and to show or hide theme selector.
