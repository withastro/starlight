...
You can apply a wider page layout without sidebars by setting `template: splash` in a page’s frontmatter.
...
By default, `<h2>` and `<h3>` headings are included in the table of contents. Change which headings levels to include using the `minHeadingLevel` and `maxHeadingLevel` options:
...
Currently Discord, GitHub, Mastodon, and Twitter are supported.
...
For open source projects in particular, this can help encourage contributions from your users.
...
Customize the styles applied to your Starlight site by providing additional CSS files to apply, modifying or extending Starlight’s default styles.
...
If you are considering using Google Fonts, we recommend following the [Fontsource set-up guide](#set-up-a-fontsource-font).
...
3. Add your `@font-face` declarations to Starlight’s `customCss` array in `astro.config.mjs`:
