...
Starlight provides three options for dark/light theme choise: `dark`, `light` and `auto`. `auto` uses users default environment settings to detect dark or ligh useage.

There are cases, when a fixed `dark` or `light` default theme should be loaded by default, also in some cases it's required to make that option fixed.
...
export default defineConfig({
	integrations: [
		starlight({
			title: 'Docs With fixed light theme',
			themeSelection: {
				showDropdown: false,
				defaultTheme: 'light',
			},
