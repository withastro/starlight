...
- If the built-in component you are reusing also contains [named slots](https://docs.astro.build/en/basics/astro-components/#named-slots), you need to transfer them as well. For example the [`TwoColumnContent`](/reference/overrides/#twocolumncontent) component uses a `right-sidebar` slot. To keep it you need to transfer it using the `<slot name="right-sidebar" slot="right-sidebar" />` syntax (see [transferring slots](https://docs.astro.build/en/basics/astro-components/#transferring-slots)).
## Use page data
