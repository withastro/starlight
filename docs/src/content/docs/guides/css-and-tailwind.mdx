...
3.  Replace the contents of the `src/styles/global.css` file scaffolded by Astro for compatibility with Starlight:
    ...
    This Tailwind theme config defines Starlight [cascade layers](#cascade-layers) order, imports Starlight’s Tailwind complementary CSS, and imports Tailwind’s theme and utility styles.
...
Additional theme config files can also be used to apply different configurations to different parts of your site, e.g. when [using Starlight at a subpath](/manual-setup/#use-starlight-at-a-subpath) or when adding [custom page](/guides/pages/#custom-pages) to your site, where you may want for example to use Tailwind’s Preflight reset styles that Starlight only use partially.

When creating new Tailwind config CSS files, import them in the pages where you want this specific configuration to apply, which is often done in a layout component so that Tailwind styles can be used on all pages sharing that layout.

The following Tailwind theme config file is used to import Tailwind without any plugin or extra configuration:
...
Such CSS file can then be imported in a layout component to apply Tailwind styles to all custom pages using that layout:
...
To learn more about Tailwind theme config, check out the [Tailwind CSS documentation](https://tailwindcss.com/docs/theme).
