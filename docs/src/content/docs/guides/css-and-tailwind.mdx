...
Tailwind v4 CSS support in Astro projects is provided by the [Tailwind Vite plugin](https://tailwindcss.com/docs/installation/using-vite).
...
Additional theme config files can also be used to apply different configurations to different parts of your site, e.g. when [using Starlight at a subpath](/manual-setup/#use-starlight-at-a-subpath) or when adding [custom pages](/guides/pages/#custom-pages) to your site.
For example, you may want to use Tailwind’s Preflight reset styles that Starlight only use partially.

When using multiple Tailwind config CSS files, import them in the pages where you want a specific configuration to apply, which is often done in a layout component so that Tailwind styles can be used on all pages sharing that layout.
...
Such a CSS file can then be imported in a layout component to apply Tailwind styles to all custom pages using that layout:
