...
3.  Create a CSS file for Tailwind’s base styles, for example at `src/tailwind.css`, including Starlight Tailwind CSS:
    ...
    ```css {4}
    /* src/tailwind.css */
    @layer starlight.base, starlight.reset, starlight.core, starlight.content, starlight.components, starlight.asides, starlight.expressive-code, starlight.utils, theme, base, components, utilities;
