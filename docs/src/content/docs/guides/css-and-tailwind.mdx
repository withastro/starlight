...
```css
body {
/* Count from -1 because page title is also a h1 element */
    counter-reset: h1 -1;
}
...
/* Exclude the page title which id is "_top" */
h1:not(#_top):before {
    content: counter(h1) ". "
}

/* Exclude the right-sidebar title which id is "starlight__on-this-page" */
h2:not(#starlight__on-this-page):before {
