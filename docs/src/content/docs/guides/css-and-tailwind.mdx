...
Additional theme config files can also be used to apply different configurations to different parts of your site, e.g. when [using Starlight at a subpath](/manual-setup/#use-starlight-at-a-subpath) or when adding [custom pages](/guides/pages/#custom-pages) to your site, where you may want for example to use Tailwind’s Preflight reset styles that Starlight only use partially.
