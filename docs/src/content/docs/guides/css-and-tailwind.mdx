...
Starlight uses [cascade layers](https://developer.mozilla.org/en-US/docs/Learn/CSS/Building_blocks/Cascade_layers) internally for managing the order of styles. This results in a more predictable CSS order and allows for easier overrides. Any custom unlayered CSS will override the default styles from the `starlight` layer.
...
The following example shows how to use the `@layer` at-rule to define a custom layer named `my-reset` applied before all Starlight layers and another custom layer named `my-overrides` that will be applied after all Starlight layers and have precedence over them:

```css "my-reset" "my-overrides"
...
- Restores essential parts of Preflight.
...
4.  Update your Astro config file to use your Tailwind base styles:
...
>
	...
	<Fragment slot="tailwind-docs">
		...
		file](#styling-starlight-with-tailwind):
	</Fragment>
</ThemeDesigner>
