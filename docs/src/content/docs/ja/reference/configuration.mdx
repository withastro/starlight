...
サイドバーはリンク、リンクのグループ、および自動生成されたエントリの配列です。サイドバー項目は、以下のプロパティのいずれかを使用して定義します。

- `link` — 特定のURL、たとえば`'/home'`や`'https://example.com'`などへの単一のリンク。リンクには`label`も必要です。

- `slug` — たとえば`'guides/getting-started'`など、内部ページへの参照。デフォルトでは、リンク先のページのタイトルがラベルとして使用されます。

- `items` — サイドバーの複数のリンク、サブグループ、および自動生成されたエントリを含む配列。グループには`label`も必要です。

- `autogenerate` — リンクとサブグループを自動的に生成するために、ドキュメントのディレクトリを指定するオブジェクト。自動生成されたエントリは、グループの`items`配列内に配置できます。
...
自動生成されたサブグループもデフォルトで展開されます。`autogenerate.collapsed`プロパティを設定して、これらを折りたたむことができます。
