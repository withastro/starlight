...
このガイドを進めるには、[既存のAstroプロジェクト](https://docs.astro.build/ja/install-and-setup/)が必要です。
...
Starlightは、v4までの従来のコンテンツコレクション機能の一部を保持する[`legacy.collectionsBackwardsCompat`フラグ](https://docs.astro.build/ja/reference/legacy-flags/#collectionsbackwardscompat)もサポートしています。これは既存のAstroプロジェクトがあり、v5.0で導入されたContent Layer APIへ移行するために、既存のコレクション設定を現状から変更できない場合に役立ちます。
...
ドキュメントプロジェクトでサーバーサイドレンダリングを有効にするために[Cloudflareアダプター](https://docs.astro.build/ja/guides/integrations-guide/cloudflare/)を使用している場合は、必ず[Wrangler設定ファイルに`nodejs_compat`互換性フラグも追加してください](https://developers.cloudflare.com/workers/runtime-apis/nodejs/#get-started)。
