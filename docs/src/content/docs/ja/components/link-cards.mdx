...
<LinkCard title="Markdownの作成" href="/guides/authoring-content/" />
```
...
[`<CardGrid>`](/components/card-grids/)コンポーネントを使用してグループ化することで、十分なスペースがある場合に複数のリンクカードを並べて表示できます。

例については、["リンクカードのグループ化"](/ja/components/card-grids/#group-link-cards)ガイドを参照してください。

## `<LinkCard>` プロパティ

**実装:** [`LinkCard.astro`](https://github.com/withastro/starlight/blob/main/packages/starlight/user-components/LinkCard.astro)

`<LinkCard>`コンポーネントは、以下のプロパティおよび他のすべての[`<a>`要素の属性](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/a)を受け入れます：
