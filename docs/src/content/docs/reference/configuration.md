...
Groups of links are expanded by default, but can be collapsed by setting the `collapsed` property to `true`. Autogenerated subgroups respect the `collapsed` property of their parent group by default but can override this behaviour using the `autogenerate.collapsed` property.
...
starlight({
  sidebar: [
    ...
    { label: 'Home', link: '/' },
    // A collapsed group labelled “Start Here” containing two links.
    {
      label: 'Start Here',
      collapsed: true,
      items: [
