...
**type:** `{ label?: string; order?: number; hidden?: boolean, badge?: string | { text: string; variant: 'blue' | 'pink' | 'green' | 'yellow' | 'purple'; } }`
...
**type:** `string | { text: string; variant: 'blue' | 'pink' | 'green' | 'yellow' | 'purple'; }`

Add a badge to the page in the sidebar when displayed in an autogenerated group of links.
When using a string, the badge will be displayed with a default `blue` variant.
Optionally, pass an object with `text` and `variant` fields to customize the badge.

```md
---
title: Page with a badge
sidebar:
  # Uses the default blue variant
  badge: New
---
```
