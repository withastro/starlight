...
**type:** `{ label?: string; order?: number; hidden?: boolean, badge?: string | { text: string; variant: 'blue' | 'pink' | 'green' | 'yellow' | 'purple' | 'default' ; } }`
...
**type:** `string | { text: string; variant: 'blue' | 'pink' | 'green' | 'yellow' | 'purple'; | 'default' }`
