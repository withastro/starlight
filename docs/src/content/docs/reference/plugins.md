...
**type:** `StarlightVirtualFrontmatter`
...
- The [`sidebar`](/reference/frontmatter/#sidebar) property is not supported as this option controls how a page is displayed in the sidebar, when using an [autogenerated link group](/reference/configuration/#sidebar) which is not applicable to virtual pages.
