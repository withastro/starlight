...
사이드바는 링크, 링크 그룹, 그리고 자동으로 생성된 항목의 배열입니다.
사이드바 항목은 다음 속성 중 하나를 사용하여 정의됩니다.

- `link` — `'/home'` 또는 `'https://example.com'`과 같이 특정 URL로 연결되는 단일 링크입니다. 링크에는 `label`도 있어야 합니다.

- `slug` — `'guides/getting-started'`와 같이 내부 페이지에 대한 참조입니다. 기본적으로 연결된 페이지의 제목이 라벨로 사용됩니다.

- `items` — 더 많은 사이드바 링크, 하위 그룹, 자동 생성 항목을 포함하는 배열입니다. 그룹에는 `label`도 있어야 합니다.

- `autogenerate` — 링크와 하위 그룹을 자동으로 생성할 문서 디렉터리를 지정하는 객체입니다. 자동 생성 항목은 그룹의 `items` 배열에 배치할 수 있습니다.
...
자동 생성된 하위 그룹도 기본적으로 펼쳐져 있습니다. `autogenerate.collapsed` 속성을 설정하여 이를 최소화할 수 있습니다.
