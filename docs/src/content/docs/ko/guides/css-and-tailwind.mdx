...
Starlight의 기본 스타일을 수정하거나 확장하기 위한 추가 CSS 파일을 제공하여 Starlight 사이트에 적용된 스타일을 사용자 정의하세요.

1. `src/` 디렉토리에 CSS 파일을 추가합니다. 예를 들어, 페이지 제목에 대해 더 넓은 기본 열의 너비와 더 큰 텍스트 크기를 설정할 수 있습니다.
...
2. `astro.config.mjs`에 있는 Starlight의 `customCss` 배열에 CSS 파일 경로를 추가하세요.
...
[Github에 있는 `props.css` 파일](https://github.com/withastro/starlight/blob/main/packages/starlight/style/props.css)에서 사이트를 사용자 정의하기 위해 Starlight에서 사용되는 모든 CSS 사용자 정의 속성을 확인할 수 있습니다.
...
- Starlight의 어두운 모드와 작동하도록 Tailwind의 `dark:` 변형을 구성합니다.
- Starlight UI에서 Tailwind [테마 색상 및 글꼴](#tailwind로-starlight-스타일링하기)를 사용합니다.
...
이미 Starlight 사이트가 있고 Tailwind CSS를 추가하려면 다음 단계를 따르세요.
...
3.  `src/tailwind.css`와 같은 Tailwind의 기본 스타일에 대한 CSS 파일을 만드세요.
...
5.  `tailwind.config.cjs`에 Starlight Tailwind 플러그인을 추가하세요.
...
Starlight의 색상 테마는 기본 사용자 정의 속성을 재정의하여 제어할 수 있습니다.
이러한 변수는 텍스트 및 배경색에 사용되는 다양한 회색 음영과 링크에 사용되는 강조 색상 및 탐색에서 현재 항목을 강조 표시하는 UI 전체에서 사용됩니다.

### 색상 테마 편집기

아래 슬라이더를 사용하여 Starlight의 강조 및 회색 색상 팔레트를 수정하세요. 어둡고 밝은 미리보기 영역에는 결과 색상이 표시되며 전체 페이지도 업데이트되어 변경 사항을 미리 볼 수 있습니다.
...
>
	<Fragment slot="css-docs">
		이 테마를 사이트에 적용하려면 프로젝트의 [사용자 정의 CSS
		파일](#사용자-정의-css-스타일)에 다음 CSS를 추가하세요.
	</Fragment>
