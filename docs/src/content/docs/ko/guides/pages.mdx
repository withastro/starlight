...
예를 들어, 다음 파일 구조는 `example.com/hello-world` 및 `example.com/ko/reference/faq`에 페이지를 생성합니다.
...
모든 Starlight 페이지는 페이지 표시 방법을 제어하기 위해 사용자 정의 가능한 [공통 프론트매터 속성 세트](/ko/ko/reference/frontmatter/)를 공유합니다.
