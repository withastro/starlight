...
[자동 생성 그룹](#자동-생성-링크) 섹션에서 자동으로 생성된 사이드바에 대해 자세히 알아보세요.
...
Starlight는 문서 디렉터리를 기반해서 자동으로 사이드바에 링크를 생성할 수 있습니다.
이는 그룹의 각 사이드바 항목을 수동으로 입력하고 싶지 않을 때 유용합니다.

기본적으로 페이지는 파일 [`id`](/ko/reference/route-data/#id)에 따라 알파벳순으로 정렬됩니다.

`autogenerate` 속성이 있는 객체를 사용하여 자동 생성 링크를 추가합니다.
`autogenerate` 구성은 사이드바 항목에 사용할 디렉터리를 지정해야 합니다. 예를 들어, 다음 구성을 사용할 수 있습니다.
...
위와 같은 프런트매터가 설정된 페이지를 포함하여 자동 생성된 링크 그룹은 다음과 같은 사이드바를 생성합니다:
...
:::note
`sidebar` 프런트매터 구성은 `slug` 속성으로 정의된 링크 및 문서 링크에만 사용됩니다. `link` 속성으로 정의된 링크에는 적용되지 않습니다.
:::

## 배지

링크 및 그룹은 라벨 옆에 배지를 표시하기 위해 `badge` 속성을 포함할 수도 있습니다.
...
`autogenerate` 구성의 `attrs` 속성을 정의하여 [자동 생성 링크](#자동-생성-링크)의 HTML 속성을 사용자 정의합니다.
	...
	sidebar: [
		{
			label: '별자리',
			autogenerate: {
				// 'constellations' 디렉터리에 대한 링크를 자동으로 생성합니다.
				directory: 'constellations',
				// 모든 자동 생성 링크의 라벨을 이탤릭체로 표시합니다.
...
<SidebarPreview
	config={[
		...
		{ label: '켄타우루스', link: '', attrs: { style: 'font-style: italic' } },
		{
			label: '계절별',
			items: [
				{
...
`autogenerate.collapsed` 속성을 `true`로 설정하여 [자동 생성된 하위 그룹](#자동-생성-링크)도 기본적으로 접히도록 설정할 수 있습니다.
