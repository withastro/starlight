...
Starlight의 [기본 제공 UI 문자열](/ko/guides/i18n/#translate-starlights-ui)과 [사용자 정의](/ko/guides/i18n/#extend-translation-schema) 및 [플러그인 제공](/ko/reference/plugins/#injecttranslations) UI 문자열에 액세스할 수 있는 통합 API는 [i18next](https://www.i18next.com/)로 구동됩니다.
...
로케일에 대한 번역 키가 존재하는지 확인하려면 번역 키를 첫 번째 인수로 전달하여 `locals.t.exists()` 함수를 사용합니다.
현재 로케일을 재정의해야 하는 경우 선택적 두 번째 인수를 전달합니다.
...
다음 예시는 현재 로케일을 읽고 이를 사용하여 현재 언어로 작성된 소개 페이지로 이동하는 링크를 생성합니다.
...
<a href={`/${Astro.currentLocale}/about`}>소개</a>
```
