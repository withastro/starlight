...
이 가이드를 따르기 위해서는 [기존 Astro 프로젝트](https://docs.astro.build/ko/install-and-setup/)가 필요합니다.
...
Starlight는 일부 v4 레거시 콘텐츠 컬렉션 기능을 유지하는 [`legacy.collectionsBackwardsCompat` 플래그](https://docs.astro.build/ko/reference/legacy-flags/#collectionsbackwardscompat)도 지원합니다.
이미 진행 중인 Astro 프로젝트가 있고, 현재 시점에서 컬렉션을 수정하여 v5.0에서 도입된 Content Layer API로 마이그레이션할 수 없는 경우 이 방법이 유용합니다.
...
문서 프로젝트에서 서버 렌더링을 활성화하기 위해 [Cloudflare 어댑터](https://docs.astro.build/ko/guides/integrations-guide/cloudflare/)를 사용하고 있다면, [Wrangler 구성 파일에 `nodejs_compat` 호환성 플래그도 추가](https://developers.cloudflare.com/workers/runtime-apis/nodejs/#get-started)했는지 확인하세요.
