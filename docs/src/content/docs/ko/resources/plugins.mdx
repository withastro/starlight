...
<ThirdPartyContent>
	<CardGrid>
		...
		<LinkCard
			...
			description="테마를 인식하는 구문 강조와 함께 클래스, 메서드, 속성, 타입 문서를 라이트 DOM에서 렌더링하는 Swagger 스타일의 API 문서 컴포넌트 세트입니다."
		/>
		<LinkCard
			href="https://github.com/andriygm/starlight-base-path/"
			title="starlight-base-path"
			description="콘텐츠 내 링크에 구성된 기본 경로를 자동으로 추가합니다."
		/>
		<LinkCard
			href="https://github.com/trueberryless-org/starlight-recipes/"
			title="starlight-recipes"
			description="레시피 웹사이트를 만들기 위한 Starlight 플러그인입니다."
		/>
		<LinkCard
			href="https://github.com/max-ostapenko/starlight-md-txt"
			title="starlight-md-txt"
			description=".md.txt URL을 통해 Starlight 문서 페이지를 에이전트가 처리하기 쉬운 원시 Markdown 형식으로 제공합니다."
		/>
		<LinkCard
			href="https://github.com/TechDocsStudio/starlight-biel"
			title="starlight-biel"
			description="Biel.ai 기반의 콘텐츠를 바탕으로 답변하는 Ask AI 챗봇을 추가하세요."
		/>
		<LinkCard
			href="https://github.com/babblebey/starlight-page-context-action"
			title="starlight-page-context-action"
			description="페이지를 Markdown으로 복사/보기, AI 채팅에서 열기, 위로 스크롤하기와 같은 유틸리티 기능들을 문서에서 항상 사용할 수 있도록 만드세요."
		/>
		<LinkCard
		...
		<LinkCard
			...
			description="페이지 간 이동을 위한 사이드 내비게이션 스트립, 플로팅 버튼, 키보드 단축키 및 스와이프 제스처를 추가합니다."
		/>
		<LinkCard
			href="https://github.com/HiDeoo/starlight-to-skills"
			title="starlight-to-skills"
			description="Starlight 문서 페이지를 검토되고, 검색 가능하며, 최신 상태로 유지되는 에이전트 스킬로 변환하세요."
		/>
	</CardGrid>
</ThirdPartyContent>
...
<ThirdPartyContent>
	<CardGrid>
		...
		<LinkCard
			...
			description="원본 파일을 직접 수정하지 않고도 Markdown 및 MDX 페이지를 편집하고, 프론트매터를 관리하며, Starlight 문서를 미리 볼 수 있는 로컬 우선 브라우저 편집기입니다."
		/>
		<LinkCard
			href="https://github.com/joesaby/astro-archify"
			title="astro-archify"
			description="JSON IR 코드 블록에서 Archify 시스템 다이어그램을 빌드 시점에 렌더링합니다. 단독 Astro 및 Starlight에서 사용할 수 있습니다."
		/>
	</CardGrid>
</ThirdPartyContent>
