...
Чтобы исключить страницу из поискового индекса, добавьте [`pagefind: false`](/reference/frontmatter/#pagefind) в метаданных страницы:
...
2. Добавьте DocSearch в конфигурацию Starlight [`plugins`](/reference/configuration/#plugins) в `astro.config.mjs` и передайте ему ваши Algolia `appId`, `apiKey` и `indexName`:
...
Добавьте перевод модального пользовательского интерфейса для вашего языка, используя встроенную [систему интернационализации](/ru/guides/i18n/#translate-starlights-ui) Starlight.
