...
<SidebarPreview
	config={[
		...
		},
		{
			label: 'Справочник',
			items: [
				{ label: 'Справочник по конфигурации', link: '/ru/reference/configuration/' },
			],
		},
	]}
/>
...
используйте свойство [`starlight.sidebar`](/reference/configuration/#sidebar) в `astro.config.mjs`.
	...
	config={[
		{
			...
			items: [
                ...
                { label: 'Интернационализация (i18n)', link: '/ru/guides/i18n/' },
				{
					label: 'дополнительно',
					items: [
						{ label: 'Структура проекта', link: '/ru/guides/project-structure/' },
					],
				},
			],
		},
...
`text` представляет содержимое для отображения (например, "New").
...
При просмотре документации на испанском языке будет сгенерирована следующая боковая панель:
...
starlight({
	sidebar: [
		{
			label: 'Руководства',
			// Не сворачивать группу "Руководства", но свернуть
            // её автоматически сгенерированные подгруппы.
			collapsed: false,
