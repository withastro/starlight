...
Узнайте больше об автоматически генерируемых боковых панелях в разделе [Автогенерируемые ссылки](#автогенерируемые-ссылки).
...
starlight({
	sidebar: [
		...
		{ label: 'Магазин Meteor', link: '/shop/' },
...
Starlight может автоматически генерировать ссылки в вашей боковой панели, основываясь на директориях в вашей документации.
Это полезно, когда вы не хотите вручную вводить каждый элемент боковой панели в группе.

По умолчанию страницы сортируются в алфавитном порядке в соответствии со свойством [`id`](/ru/reference/route-data/#id).

Добавьте автогенерируемые ссылки, используя объект со свойством `autogenerate`.
Ваша конфигурация `autogenerate` должна указывать `directory`, которая будет использоваться для записей боковой панели.
Например, со следующей конфигурацией:
...
starlight({
	sidebar: [
		{
			label: 'Созвездия',
			// Автогенерация ссылок для директории 'constellations'.
...
Группа с автогенерируемыми ссылками, включающая страницу с вышеуказанными метаданными, сгенерирует следующую боковую панель:
...
:::note
Параметр `sidebar` в метаданных используется только для автогенерируемых ссылок и ссылок на документы, заданных с помощью свойства `slug`. Он не применяется к ссылкам, заданным с помощью свойства `link`.
:::

## Значки

Ссылки и группы также могут включать свойство `badge` для отображения значка рядом с текстом ссылки.
...
Настройте HTML-атрибуты всех [автогенерируемых ссылок](#автогенерируемые-ссылки), указав свойство `attrs` в конфигурации `autogenerate`.
...
starlight({
	sidebar: [
		{
			autogenerate: {
				// Генерируем ссылки для директории 'constellations'.
				directory: 'constellations',
				// Выделяем курсивом ярлыки всех автогенерируемых ссылок.
...
<SidebarPreview
	config={[
		...
		{ label: 'Центавр', link: '', attrs: { style: 'font-style: italic' } },
		{
			label: 'seasonal',
			items: [
				{
...
[Автогенерируемые подгруппы](#автогенерируемые-ссылки) также могут сворачиваться по умолчанию, если установить свойство `autogenerate.collapsed` в значение `true`.
			...
			items: [
				{
					autogenerate: {
						directory: 'constellations',
						// Сворачивание автогенерируемых подгрупп по умолчанию.
						collapsed: true,
					},
				},
			],
