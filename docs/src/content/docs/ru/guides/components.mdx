...
Используйте дополнительный атрибут `icon`, чтобы включить один из [встроенных значков Starlight](#все-иконки) рядом с меткой.
...
Вставки (также известные как «предостережения» или «вызовы») полезны для отображения второстепенной информации рядом с основным содержанием страницы.
...
Используйте компонент `<Code>` для рендеринга выделенного синтаксиса кода, когда использование [блока кода Markdown](/ru/guides/authoring-content/#code-blocks) невозможно, например, для рендеринга данных, поступающих из внешних источников: файлов, баз данных или API.
