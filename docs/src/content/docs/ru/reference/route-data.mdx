...
**Тип:** <code>Array&lt;<a href="#sidebarlink-type">SidebarLink</a> | <a href="#sidebargroup-type">SidebarGroup</a>&gt;</code>

Элементы боковой панели навигации сайта для этой страницы.
Каждая запись представляет собой либо ссылку боковой панели, либо группу боковой панели.

#### Тип `SidebarLink`

Записи ссылок боковой панели представляют собой ссылку, отображаемую в боковой панели, и имеют следующие свойства:
...
Подробнее см. в [справочнике по пропсам компонента `<Badge>`](/ru/components/badges/#параметры-badge).

##### `autogenerate`

**Тип:** `{ directory: string } | undefined`

Для ссылок, созданных из [автогенерируемой записи боковой панели](/ru/guides/sidebar/#автогенерируемые-ссылки), `directory` соответствует настроенному значению [`autogenerate.directory`](/ru/reference/configuration/#sidebar).

Если конфигурация [`sidebar`](/ru/reference/configuration/#sidebar) не задана, Starlight генерирует записи для всех страниц документации и использует пустую строку (`''`) в качестве значения `directory`.

#### Тип `SidebarGroup`

Записи групп боковой панели представляют собой группу ссылок, отображаемую в боковой панели, и имеют следующие свойства:
...
Для групп, созданных из [автогенерируемой записи боковой панели](/ru/guides/sidebar/#автогенерируемые-ссылки), `directory` соответствует настроенному значению [`autogenerate.directory`](/ru/reference/configuration/#sidebar).
