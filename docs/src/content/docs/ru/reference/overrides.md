...
Используйте вместо него свойство [`id`](#id), перейдя на новый Content Layer API с помощью [`docsLoader`](/ru/manual-setup/#настройка-коллекций-контента) от Starlight.

#### `id`

**тип:** `string`

Слаг или уникальный идентификатор этой страницы, основанный на имени файла, если используется флаг [`legacy.collections`](https://docs.astro.build/ru/reference/legacy-flags/#collections).
...
Подробнее о форме этого объекта можно узнать из справочника [Коллекция типов записей Astro](https://docs.astro.build/ru/reference/modules/astro-content/#collectionentry).
