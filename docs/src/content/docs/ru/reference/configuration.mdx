...
Метка для этого языка, которую нужно показывать пользователям, например, в переключателе языков. Чаще всего вы хотите, чтобы это было название языка в том виде, в котором пользователь этого языка ожидал бы его прочитать, например `"English"`, `"العربية"` или `"Русский"`.

##### `lang`

**тип:** `string`

Метка BCP-47 для этого языка, например, `"en"`, `"ar"` или `"ru"`. Если не задано, то по умолчанию будет использоваться имя каталога языка. Языковые теги с региональными подтегами (например, `"pt-BR"` или `"en-US"`) будут использовать встроенные переводы пользовательского интерфейса для своего базового языка, если не будут найдены переводы для конкретного региона.
...
starlight({
	head: [
		// Пример: добавьте тег скрипта аналитики Fathom.
		{
			tag: 'script',
			attrs: {
...
starlight({
  favicon: '/images/favicon.svg',
  head: [
    // Добавляем резервный значок ICO для Safari
    {
      tag: 'link',
      attrs: {
        rel: 'icon',
...
**тип:** [`StarlightPlugin[]`](/ru/reference/plugins/#quick-api-reference)
...
Посетите [витрину плагинов](/ru/resources/plugins/#plugins), чтобы увидеть список доступных плагинов.
