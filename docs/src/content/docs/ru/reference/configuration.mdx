...
Боковая панель — это массив ссылок, групп ссылок и автогенерируемых записей.
Элементы боковой панели определяются с помощью одного из следующих свойств:

- `link` — одиночная ссылка на конкретный URL, например `'/home'` или `'https://example.com'`. У ссылки также обязательно должен быть `label`.

- `slug` — ссылка на внутреннюю страницу, например `'guides/getting-started'`. По умолчанию в качестве метки используется заголовок связанной страницы.

- `items` — массив, содержащий дополнительные ссылки боковой панели, подгруппы и автогенерируемые записи. У группы также обязательно должен быть `label`.

- `autogenerate` — объект, указывающий на конкретную директорию для автоматической генерации ссылок и подгрупп. Автогенерируемые записи можно размещать в массиве `items` группы.
...
Автогенерируемые подгруппы также раскрываются по умолчанию. Установите свойство `autogenerate.collapsed`, чтобы сворачивать их.
...
type SidebarItem =
	...
	| {
			// Автогенерируемые ссылки и подгруппы
			autogenerate: {
				directory: string;
				collapsed?: boolean;
				attrs?: Record<string, string | number | boolean | undefined>;
