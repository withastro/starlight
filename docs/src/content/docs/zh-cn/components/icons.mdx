...
使用 `<Icon>` 组件来显示图标。
图标需要将 [`name`](#name) 设置为 [Starlight 的内置图标之一](/reference/icons/#all-icons)，并且可以选择是否包含 [`label`](#label) 来为屏幕阅读器提供上下文。
...
要显示的图标名称设置为 [Starlight 的内置图标之一](/reference/icons/#all-icons)。
