...
通过将 [`icon`](#icon) 属性设置为 [Starlight 内置图标之一](/zh-cn/reference/icons/#所有图标) 的名称，以覆盖默认的旁白图标。
...
**实现：** [`Aside.astro`](https://github.com/withastro/starlight/blob/main/packages/starlight/src/user-components/Aside.astro)
...
旁白可以包含一个 `icon` 属性，设置为 [Starlight 内置图标之一](/zh-cn/reference/icons/#所有图标) 的名称。
