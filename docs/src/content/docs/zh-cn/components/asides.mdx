...
通过将 [`icon`](#icon) 属性设置为 [Starlight 内置图标之一](/reference/icons/#所有图标) 的名称，以覆盖默认的旁白图标。
...
**类型：** [`StarlightIcon`](/reference/icons/#starlighticon-类型)

旁白可以包含一个 `icon` 属性，设置为 [Starlight 内置图标之一](/reference/icons/#所有图标) 的名称。
