...
通过在 `autogenerate` 配置中定义 `attrs` 属性，为所有[自动生成的分组](#自动生成的分组)自定义 HTML 属性。
...
[自动生成的子组](#自动生成的分组) 也可以通过将 `autogenerate.collapsed` 属性设置为 `true` 来默认折叠。
