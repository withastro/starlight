...
在 [自动生成的链接](#自动生成的链接) 章节了解更多关于自动生成侧边栏的内容。
...
Starlight 可以根据文档目录在侧边栏中自动生成链接。
当你不想手动输入分组中的每个侧边栏项目时，这很有用。

默认情况下，页面按照文件 [`id`](/zh-cn/reference/route-data/#id) 的字母顺序排序。

使用具有 `autogenerate` 属性的对象添加自动生成的链接。
`autogenerate` 的配置必须指定用于侧边栏条目的 `directory`。
例如，使用以下配置：
...
侧边栏 frontmatter 选项允许你设置 [自定义标签](/zh-cn/reference/frontmatter/#label)，使用 [自定义属性](/zh-cn/reference/frontmatter/#attrs)，为链接添加 [徽章](/zh-cn/reference/frontmatter/#badge)，[隐藏](/zh-cn/reference/frontmatter/#hidden) 侧边栏中的链接，或者定义 [自定义排序权重](/zh-cn/reference/frontmatter/#order)。
...
:::note
`sidebar` frontmatter 配置仅用于自动生成的链接和使用 `slug` 属性定义的文档链接。它并不适用于使用 `link` 属性定义的链接。
:::
...
通过在 `autogenerate` 配置中定义 `attrs` 属性，为所有[自动生成的链接](#自动生成的链接)自定义 HTML 属性。
单个页面可以使用 [`sidebar.attrs` frontmatter 字段](/zh-cn/reference/frontmatter/#attrs) 指定自定义属性，这些属性将与 `autogenerate.attrs` 配置合并。
...
<SidebarPreview
	config={[
		{ label: '船底座', link: '', attrs: { style: 'font-style: italic' } },
		{ label: '半人马座', link: '', attrs: { style: 'font-style: italic' } },
		{
			label: 'seasonal',
			items: [
				{
...
[自动生成的子组](#自动生成的链接) 也可以通过将 `autogenerate.collapsed` 属性设置为 `true` 来默认折叠。
