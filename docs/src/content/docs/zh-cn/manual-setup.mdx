...
要遵循本指南，你需要一个 [现有的 Astro 项目](https://docs.astro.build/zh-cn/install-and-setup/)。
...
创建或更新内容配置文件，添加一个使用 Starlight 的 [`docsLoader`](/zh-cn/reference/configuration/#docsloader) 和 [`docsSchema`](/zh-cn/reference/configuration/#docsschema) 的 `docs` 集合：
...
Starlight 也支持 [`legacy.collectionsBackwardsCompat` 标志](https://docs.astro.build/zh-cn/reference/legacy-flags/#collectionsbackwardscompat)，该标志保留了 v4 版本中的一些旧版内容集合功能。
这对于你有一个现有的 Astro 项目，目前无法对集合进行任何更改以迁移到 v5.0 中引入的 Content Layer API 的情况非常有用。
...
Starlight 使用基于文件的路由，这意味着 `src/content/docs/` 中的每个 Markdown、MDX 或 Markdoc 文件都将变成你站点上的一个页面。前置元数据（上面示例中的 `title` 和 `description` 字段）可以更改每个页面的显示方式。
在 [frontmatter 指南](/zh-cn/reference/frontmatter/) 中查看所有可用选项。
...
如果你正在使用 [Cloudflare 适配器](https://docs.astro.build/zh-cn/guides/integrations-guide/cloudflare/) 在文档项目中启用服务端渲染，请确保同时 [将 `nodejs_compat` 兼容性标志添加到你的 Wrangler 配置文件中](https://developers.cloudflare.com/workers/runtime-apis/nodejs/#get-started)。
