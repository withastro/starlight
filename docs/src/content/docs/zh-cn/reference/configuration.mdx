...
侧边栏是一个由链接、链接组和自动生成条目组成的数组。
侧边栏条目通过以下属性之一来定义：

- `link` — 指向特定 URL 的单个链接，例如 `'/home'` 或 `'https://example.com'`。链接还必须具有 `label`。

- `slug` — 指向一个内部页面的引用，例如 `'guides/getting-started'`。链接页面的标题将默认用作标签。

- `items` — 包含更多侧边栏链接、子组和自动生成条目的数组。分组还必须具有 `label`。

- `autogenerate` — 一个用于指定文档目录以自动生成链接和子组的对象。自动生成的条目可以放置在分组的 `items` 数组中。
...
自动生成的侧边栏组按文档名字母顺序排序。
例如，从 `astro.md` 生成的页面将显示在 `starlight.md`页面上方。
...
**类型：** `{ headingLinks?: boolean; processedDirs?: string[] }`  
...
默认情况下，仅处理使用 Starlight 的 [`docsLoader()`](#docsloader) 加载的 Markdown 和 MDX 内容。
支持相对于项目根目录的本地目录，例如 `'./src/data/comments/'`。

Starlight 的处理内容包括对[可点击标题锚点链接](#headinglinks)、[旁白 Markdown 指令语法](/zh-cn/guides/authoring-content/#旁白)以及代码块的 RTL 支持。
如果你正在通过 `<StarlightPage>` 组件在[自定义页面](/zh-cn/guides/pages/#自定义页面)中通过自定义内容集合渲染内容，并希望 Starlight 的 Markdown 处理也应用于该内容，那么这个选项非常有用。

```js
starlight({
	markdown: {
		// 处理位于 `src/data/reviews/` 目录中 `reviews` 内容集合的 Markdown 文件。
...
你可以通过在 Starlight 的 `expressiveCode` 选项中设置任何标准的 [Expressive Code 配置选项](https://expressive-code.com/reference/configuration/) 以及一些特定于 Starlight 的属性。
...
设置用于代码块样式的主题。
有关支持的主题格式的详细信息，请参阅 [Expressive Code `themes` 文档](https://expressive-code.com/guides/themes/)。
...
在 `head` 中的条目会直接转换为 HTML 元素，并不会经过 Astro 的 [脚本](https://docs.astro.build/zh-cn/guides/client-side-scripts/#脚本处理) 或 [样式](https://docs.astro.build/zh-cn/guides/styling/#在-astro-进行设计) 处理。
如果你需要导入本地资源，如脚本、样式或图片，请[重写 Head 组件](/zh-cn/guides/overriding-components/#复用内置组件)。
...
Starlight 使用 Astro [内容集合](https://docs.astro.build/zh-cn/guides/content-collections/) 来加载你的内容。
Starlight 的内容加载器和 schema 帮助按要求配置集合。
...
Starlight 从 `@astrojs/starlight/loaders` 模块导出以下 [Astro 加载器](https://docs.astro.build/zh-cn/reference/content-loader-reference/)，以简化配置内容集合。

#### `docsLoader()`

`docsLoader()` 从 `src/content/docs/` 目录加载本地的 Markdown、MDX 和 Markdoc 文件。
以下划线 (`_`) 开头的文件名将被忽略。
...
```js
docsLoader({
	// 删除 `.md` 或 `.mdx` 扩展名，但不处理文件名。
	generateId: ({ entry }) => entry.split('.').slice(0, -1).join('.'),
}),
```

参阅 [Astro 文档中的 `generateId()`](https://docs.astro.build/zh-cn/reference/content-loader-reference/#generateid) 了解更多详情。

#### `i18nLoader()`

`i18nLoader()` 从 `src/content/i18n/` 目录加载本地的 JSON 和 YAML 文件。
以下划线 (`_`) 开头的文件名将被忽略。
...
当前没有可用于配置 `i18nLoader()` 的选项。

### 模式

Starlight 从 `@astrojs/starlight/schema` 模块提供以下[内容集合 schema](https://docs.astro.build/zh-cn/guides/content-collections/#定义集合模式schema)。
...
参阅 [“自定义 frontmatter schema”](/zh-cn/reference/frontmatter/#自定义-frontmatter-schema) 了解更多使用 `extend` 选项的详细信息。
...
参阅 [“扩展翻译 schema”](/zh-cn/guides/i18n/#拓展翻译-schema) 了解更多使用 `extend` 选项的详细信息。
