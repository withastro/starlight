...
Jika Anda perlu menyesuaikan judul untuk setiap bahasa, Anda dapat meneruskan objek ke [`title`](/reference/configuration/#title-required) di opsi Starlight:
...
3. Tambahkan terjemahan untuk properti yang ingin Anda terjemahkan ke file JSON. Terjemahkan hanya nilai-nilainya, biarkan nama propertinya dalam bahasa Inggris (misalnya, `"search.label": "Buscar"`).
   ...
   ```json
   {
   	"pagefind.clear_search": "Bersihkan",
   	"pagefind.load_more": "Muat lebih banyak hasil",
   	"pagefind.search_label": "Cari website ini",
   	"pagefind.filters_label": "Filter",
   	"pagefind.zero_results": "Tidak ada hasil untuk [SEARCH_TERM]",
   	"pagefind.many_results": "[COUNT] hasil untuk [SEARCH_TERM]",
   	"pagefind.one_result": "[COUNT] hasil untuk [SEARCH_TERM]",
   	"pagefind.alt_search": "Tidak ada hasil untuk [SEARCH_TERM]. Menampilkan hasil untuk [DIFFERENT_TERM] sebagai gantinya",
   	"pagefind.search_suggestion": "Tidak ada hasil untuk [SEARCH_TERM]. Coba salah satu pencarian berikut:",
   	"pagefind.searching": "Mencari [SEARCH_TERM]..."
   }
   ```
